...
public class JavaFXUI {
    ...
    private void buildWindow(Stage stage) {
        TextField textField = new TextField();
        textField.setPromptText("输入表名");

        ComboBox<ConnectionConfig> connectionBox = new ComboBox<>();
        connectionBox.setPrefWidth(180);
        refreshConnections(connectionBox);
        ...
        searchButton.setOnAction(event -> search(stage, textField, table, currentConnection(nameField, urlField,
                usernameField, passwordField, driverField, schemaField)));
    ...
    private void buildTableWindow(Stage stage, ConnectionConfig connectionConfig) {
        ...
        HBox topControls = new HBox(5, textField, searchButton, newWindowButton);
        HBox.setHgrow(textField, Priority.ALWAYS);
        ...
        VBox vbox = new VBox(5, connectionPane, topControls, table, bottomControls);
        VBox.setVgrow(table, Priority.ALWAYS);
